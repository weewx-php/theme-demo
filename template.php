...
<body data-texts="<?= htmlspecialchars(json_encode($theme->texts, JSON_THROW_ON_ERROR), ENT_QUOTES, 'UTF-8') ?>">
<a class="skip" href="#weather"><?= $theme->html('Skip to weather') ?></a>
<div class="page">
    <header class="site-header">
        <a class="brand" href="?range=24h" aria-label="<?= $theme->html('Atmos weather overview') ?>">
    ...
    <?php else:
        ...
        <section class="overview" aria-label="<?= $theme->html('Current readings') ?>">
            <article class="hero panel" data-weather="<?= View::escape($scene['kind']) ?>" data-phase="<?= View::escape($scene['phase']) ?>" data-wind="<?= $scene['wind'] ?>" data-temperature="<?= is_numeric($temperature) ? sprintf('%.2F', (float) $temperature) : '' ?>" data-zone="<?= View::escape($view->zone->getName()) ?>" data-latitude="<?= $view->latitude === null ? '' : sprintf('%.4F', $view->latitude) ?>">
                ...
                <div class="hero-top"><span class="eyebrow"><?= $theme->html('Air temperature') ?> <span>/ 2 m</span></span><span class="hero-tag" data-scene-tag<?= $scene['source'] === '' ? ' hidden' : '' ?>><?= View::escape($scene['source']) ?></span></div>
                <div class="hero-reading"><span class="temperature" data-value="temperature"><?= $view->number('temperature') ?></span><span class="temperature-unit">°C</span></div>
                ...
                <div class="hero-bottom">
                    ...
                    <div class="hero-foot"><span><span class="tiny-cross">+</span> <span data-phase-label><?= $scene['phase'] === 'night' ? $theme->html('Night') : ($scene['phase'] === 'twilight' ? $theme->html('Twilight') : $theme->html('Day')) ?></span><span class="hero-foot-divider">/</span><?= View::escape($view->zone->getName()) ?></span><span><?= $view->altitude === null ? '' : number_format($view->altitude, 0, $theme->language === 'de' ? ',' : '.', $theme->language === 'de' ? '.' : ',') . $theme->html(' m above sea level') ?></span></div>
                </div>
            </article>
            <div class="metric-grid">
                <article class="metric-card humidity-card" aria-labelledby="humidity-title">
                    ...
                    <div class="metric-foot"><?= $theme->html('Dew point') ?> <strong><span data-value="dewpoint"><?= $view->number('dewpoint') ?></span> °C</strong></div>
                </article>
                <article class="metric-card wind-card" aria-labelledby="wind-title">
                    ...
                    <div class="metric-value"><span data-value="wind"><?= $view->number('wind') ?></span><span class="unit">km/h</span></div>
                    ...
                    <div class="metric-foot"><?= $theme->html('Gusts') ?> <strong><span data-value="gust"><?= $view->number('gust') ?></span> km/h</strong></div>
                </article>
                <article class="metric-card pressure-card" aria-labelledby="pressure-title">
                    ...
                    <div class="metric-value"><span data-value="pressure"><?= $view->number('pressure') ?></span><span class="unit">hPa</span></div>
                ...
                <article class="metric-card rain-mini-card" aria-labelledby="rain-mini-title">
                    ...
                    <div class="metric-value"><span data-value="rainDay"><?= $view->number('rainDay') ?></span><span class="unit">mm</span></div>
        ...
        <div class="content-grid">
            <section class="panel temperature-panel" aria-labelledby="temperature-title">
                <div class="panel-heading"><div class="panel-label"><?= $icon('chart') ?><h3 id="temperature-title"><?= $theme->html('Temperature') ?></h3></div><nav class="range" aria-label="<?= $theme->html('Range') ?>"><a href="?range=24h#analysis"<?= $view->range === '24h' ? ' aria-current="true"' : '' ?>>24 h</a><a href="?range=7d#analysis"<?= $view->range === '7d' ? ' aria-current="true"' : '' ?>><?= $theme->html('7 days') ?></a></nav></div>
                <div class="chart-headline"><span class="chart-reading"><span data-value="temperature"><?= $view->number('temperature') ?></span><span class="unit">°C</span></span><span class="legend"><i></i><?= $view->range === '24h' ? $theme->html('15-minute average') : $theme->html('Hourly average') ?></span></div>
                ...
                <div class="climate-reference">
                    ...
                    <span><?= $theme->html('Daily average') ?> <?= View::escape(is_string($climate->meta['date'] ?? null) ? substr($climate->meta['date'], 8, 2) . '.' . substr($climate->meta['date'], 5, 2) . '.' : '') ?> · <?= is_int($climate->meta['start_year'] ?? null) ? $climate->meta['start_year'] : '' ?>–<?= is_int($climate->meta['end_year'] ?? null) ? $climate->meta['end_year'] : '' ?>: <strong><?= $climate->value('outTemp')->to('degree_C')->html() ?></strong></span>
                    <span><?= $theme->html('Average minimum') ?> <?= $climate->value('outTempMin')->to('degree_C')->html() ?> <?= $theme->html('· Average maximum') ?> <?= $climate->value('outTempMax')->to('degree_C')->html() ?></span>
                ...
                <?php if ($chart['paths'] === []): ?><div class="chart-empty" role="status"><?= ($view->data['temperature' . $view->range]->status ?? '') === 'pending' ? $theme->html('Calculating history …') : $theme->html('No historical data') ?></div><?php else: ?>
                    <div class="chart-wrap" data-chart tabindex="0" role="group" aria-label="<?= $theme->html('Temperature chart. Use arrow keys to select readings.') ?>">
                    ...
                    <svg class="temperature-chart" viewBox="0 0 780 256" role="img" aria-labelledby="chart-title chart-description">
                        <title id="chart-title"><?= $theme->html('Temperature in degrees Celsius') ?></title><desc id="chart-description"><?= View::escape($view->date($chart['start']) . $theme->html(' to ') . $view->date($chart['end'])) ?><?= $theme->html('. Readings are available in the table below.') ?></desc>
                    ...
                    <details class="chart-data"><summary><?= $theme->html('Readings') ?> <?= $icon('arrow') ?></summary><div class="table-scroll"><table><caption><?= $theme->html('Temperature ·') ?> <?= $view->range === '24h' ? $theme->html('15-minute average') : $theme->html('Hourly average') ?></caption><thead><tr><th scope="col"><?= $theme->html('Time') ?></th><th scope="col">°C</th></tr></thead><tbody>
            ...
            <?php if ($forecastDays === []): ?><div class="chart-empty"><?= $theme->html('No forecast available') ?></div><?php else:
                ...
                foreach ($forecastDays as $forecastDay) {
                    foreach (['outTempMin', 'outTempMax'] as $field) {
                        $bound = $forecastDay->value($field)->to('degree_C')->raw;
                        if (is_numeric($bound)) {
                            $bounds[] = (float) $bound;
                        }
                    }
                ...
                <?php foreach ($forecastDays as $i => $forecastDay):
                    ...
                    $min = $forecastDay->value('outTempMin')->to('degree_C');
                    $max = $forecastDay->value('outTempMax')->to('degree_C');
                    $rainValue = $forecastDay->value('rain')->to('mm');
                    ...
                    <div class="forecast-temperature"><span><?= $forecastNumber($min) ?>°</span><svg viewBox="0 0 100 10" preserveAspectRatio="none" aria-hidden="true"><rect class="forecast-track" x="0" y="2" width="100" height="6" rx="3"/><?php if (is_numeric($min->raw) && is_numeric($max->raw)): ?><rect x="<?= max(0, ($min->raw - $forecastMin) / ($forecastMax - $forecastMin) * 90) ?>" y="2" width="<?= max(6, ($max->raw - $min->raw) / ($forecastMax - $forecastMin) * 90) ?>" height="6" rx="3" fill="url(#day-range)"/><?php endif; ?></svg><strong><?= $forecastNumber($max) ?>°</strong></div>
                    <div class="forecast-details"><span><?= $icon('drop') ?><?= $forecastNumber($rainValue, 1) ?> <small>mm</small></span><span><?= $icon('wind') ?><?= $forecastNumber($forecastDay->value('windGust')->to('km_per_hour')) ?> <small>km/h</small></span></div>
        ...
        <div class="rain-records-grid">
            <section class="panel rain-panel" aria-labelledby="rain-title">
                ...
                <div class="rain-summary"><div class="rain-total"><span data-value="rainDay"><?= $view->number('rainDay') ?></span><span class="unit">mm</span><small><?= $theme->html('Day') ?></small></div><dl class="rain-totals"><div><dt><?= $theme->html('Month') ?></dt><dd><span data-value="rainMonth"><?= $view->number('rainMonth') ?></span> <span>mm</span></dd></div><div><dt><?= $theme->html('Year') ?></dt><dd><span data-value="rainYear"><?= $view->number('rainYear') ?></span> <span>mm</span></dd></div></dl></div>
...
$record = $wettest->periods->points[0] ?? null; ?>
        ...
        <section class="station-strip" id="station" aria-label="<?= $theme->html('Station information') ?>"><div class="station-strip-title"><?= $icon('station') ?><span><?= View::escape($title) ?><small>weewx-php</small></span></div><div class="station-coordinates"><?php if ($view->latitude !== null && $view->longitude !== null): ?><span><?= number_format(abs($view->latitude), 4) ?>° <?= $view->latitude < 0 ? 'S' : 'N' ?></span><span><?= number_format(abs($view->longitude), 4) ?>° <?= $view->longitude < 0 ? 'W' : 'E' ?></span><?php endif; ?><span><?= $view->altitude === null ? '' : number_format($view->altitude, 0, $theme->language === 'de' ? ',' : '.', $theme->language === 'de' ? '.' : ',') . $theme->html(' m above sea level') ?></span></div></section>
